...
/// ⚠️ **wasm 文件的 sha 会随「注释行数」变化**——不是行为变，是 rustc 在 wasm 里嵌了
/// **调试行号表**（实测：加 4 行注释 → 文件哈希变、而 60 seed 的**流哈希不变**）。
/// 故本文件**不引用自己的构建哈希**（那是一个不动点问题，永远对不上，见
/// `design.md` §D11）；**交付产物的指纹以 `viewer/data/benchmark-baseline.json` 的
