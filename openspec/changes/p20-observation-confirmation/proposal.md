...
本 change 在 `captured` 与诊断之间插入「事件锚定确认」步：让模型（或用户）从窗口事件列表里选出几个候选事件，经用户确认锚定后再进入诊断。该步在 **queue-only 流程**下强制（未确认前诊断不启动），非 queue-only 流程下为可选增强（诊断随提交自动启动，确认步仍可用于给诊断提供锚点）。设计已 grill 定稿，见 `.scratch/issues/12-observation-confirmation.md`。
