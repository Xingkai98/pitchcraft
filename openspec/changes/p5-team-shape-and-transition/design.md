...
- **窗口起算点（审阅 p5）**：tackle 成功 tick 即武装 transition 状态（全队前压/回撤立即生效）；**射门被扑救同理——save 高亮结束（门将扑住）tick 起算**（与 tackle 成功对称）；"持球者前插"目标等新持球者拾取松散球后才激活（P4 D11 时序）。
- **窗口 vs 拾取时序（审阅 p5）**：transition 窗口（4 tick）从触发 tick 起算，不因松散球延长；P4 保证松散球拾取在 `LOOSE_MAX_TICKS = 2` 内完成（< 窗口 4 tick）→ "持球者前插"目标必在窗口内激活，两机制无矛盾。
...
- **transition 行为**：新进攻方持球者目标前移（高速推进，经 main 表达）、全队前压（队形偏移放大）；新防守方整体回撤 + 就近 2 名外场防守者收缩（close_down，覆盖队形目标；**松散球期间 close_down 目标 = 松散球位置**，新持球者拾取后切换为持球者）。
