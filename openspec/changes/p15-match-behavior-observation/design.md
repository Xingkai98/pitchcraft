...
4. 证明 recorder on/off 不改变正式事件、输出和 RNG。**RNG 面只能是间接证据**：正式事件流与
   golden 基线逐字节相同（决策由 rng 驱动）。比赛 rng 是 `match_events` 的局部变量、返回即
   丢弃，外部没有 raw cursor/sequence 观测工件——详见 `tasks.md` 的 Slice 3 与「口径纪律」。
