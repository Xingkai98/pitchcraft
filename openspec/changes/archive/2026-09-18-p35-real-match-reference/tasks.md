...
> 归档注记（2026-09-18）：以下两项**已由后续 change 完成**，保留原文以存溯源。
> 它们在本 change 里就被明确划到范围外，故本 change 自身 27/27 完成。

- [x] 量化指标对比：从真实数据与引擎数据**各算同一组统计量**（阵型宽度/纵深、全队重心、
      ...
      → **由 P36（`p36-match-benchmark`）完成**：`viewer/match-metrics.js` 统一帧表示 +
      指标纯函数，两侧共用一份实现，口径由测试守护。
      ⚠️ 原文的「纵深 49–58m」是 **`max-min` 口径**下的观测；P36 换 `trim1` 后同批数据的
      真实纵深为 **25.2m**——估计量的选择本身成了 P36/P37 两轮审阅的主题，不要拿这两个数
      直接与后续基线比较。
- [x] 扩大样本：接入许可更清晰的公开数据集（如 SkillCorner，MIT），并处理其外推点标记
      （`is_detected`）——外推点坐标看起来正常但不可信，需标注或弱化显示。
      → **由 P37（`p37-skillcorner-corpus`）完成**：接入 SkillCorner opendata（20 场，MIT），
      样本 2 场 → 20 场、基线 13 窗 → 152 窗；`is_detected` 原样透传
      （球员点坐标数组带第三位 `1`、球走 `ballFill:'extrapolated'`），
      指标层口径显式声明并由测试守护。原文说的「标注或弱化显示」按
      「如实暴露、不修饰」实现为**透传 + 显式口径**，而非在数据里抹掉。
