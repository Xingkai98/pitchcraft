...
**最终引擎实测抢断 9.29/场（200 seed×90min，L1 窗口 401..600），比 P30 基线的 5.60 上升 66%**，不是下降。
原因（变异实验定位，非推测）：主因是 **D3 的第四处接入**（无压久持 → 出球档）——
把该档关闭（只此一处改动）后抢断回落到 **4.54/场**（接近基线 5.6），说明它贡献了增量中的 ~4.8/场。
机制：该档让 carrier 在无压停滞时也**承诺传球动作**（pass 候选 219 → 404/场，实测），
...
（实测 7.95/9.29 = 0.855，带 [1.0,1.8] → [0.5,1.5]）。同一原则适用于删源后其余频带
（如角球——原 12% 槽位来源被删，实测 3.71 → 1.98，带 [2,9] → [1.0,7.0]）。

**注**：以上为最终引擎（含接入点③时序修复 + `OUT_CHANNEL_RISK_GAIN` 0.062 重标定）的实测值；
接入点③修复前的中间值（9.40 / 7.85 / 0.835）见审阅记录 `.p31-progress.md`。
