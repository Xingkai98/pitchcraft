...
`nearest_in_team(st, target, team)` 在「该队外场球员全部罚下」这一退化态下返回 `-1`。其 12 个调用点把返回值存进状态（`st.restart_prep.player` / `st.loose.chaser` / 角球 battle 等），后续以 `st.pos[player as usize]` 索引 —— `player == -1` 时越界 panic。
