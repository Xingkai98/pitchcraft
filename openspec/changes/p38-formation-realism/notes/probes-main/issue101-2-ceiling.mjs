...
const CAP = 6000;
// ── GBM 的算力注记（踩过三轮，留档）────────────────────────────────────
// 纯 JS 树实现，本机 4 核。三轮优化后的实测（80 棵、深 3、50k 行）：
//   v1 每节点重排序            3.4 s/棵 → 单折 >10 分钟
//   v2 每棵树子样本 presort    1.7 s/棵
//   v3 **presort 整份训练集一次、全树复用**  **0.19 s/棵**（15 s / 80 棵）
// 现在 20 折 LOOMO × 2（真实/安慰剂）× 2 节 ≈ 80 次拟合 × 15s ≈ **20 分钟**，可行。
// 仍保留 CAP=6000 的下采样（每场等间隔），使单次拟合稳定在 ~15s。
// GBM 折数 = 5（轮转分组，每折留 4 场做测试）。**为什么不是 20 折 LOOMO**：
// 实测单次拟合（80 棵 × 深 3 × 61k 训练行）≈ 3.5 分钟；20 折 × 2（真实/安慰剂）× 2 节
// = 80 次 ≈ 4.7 小时，不可行。5 折 × 2 × 2 = 20 次 ≈ 70 分钟，可接受。
// **这仍是按场分组**（同一位球员的样本不跨折），只是每折测试 4 场而非 1 场——
// 泛化读数的语义不变，方差略大。线性模型（§2.1）保留完整的 20 折 LOOMO。
const GBM_FOLDS = 5;
...
say('> ⚠ 树深 3 / 80 棵是**算力取舍**（纯 JS，本机 4 核）→ GBM 读数**略低估**上限；');
