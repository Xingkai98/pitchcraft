...
//
// 代数上可以证明这**不是**同义反复：若 10 人的 y 互相独立（方差 σ²），则
//   Cov(DV_i, F_ij) = Cov(y_i,F) − Cov(mateY_i,F) = 0 − 0 = **0**
// （两项各自为 0，各含一个 σ²/(n−1) 抵消）。即共享参考系**不产生任何底噪**。
// ⚠ 第一版把这里写成 "= 0.111σ²、R² = 0.01"，那是把 **Cov(DV_i, DV_j)**（两球员之间，
//   且是负的）误搬到了 (DV, 邻居因子) 这一对上——第三轮审阅改正。
// 真实球队里队友是**正相关**的——那才是信号。问题是：实测 R² 0.64–0.70 是否只是随机配对？
//
...
say('（耗时行已删——非确定量，会让 out/ 的逐字节比对失败）\n');
...
// ── 6.3 解析底噪 ────────────────────────────────────────────────────────
say('## 6.3 解析底噪（对照）\n');
say('若 10 人的 y **互相独立**（方差 σ²），共享参考系造成的伪影可解析求出——**它是 0**：');
say('');
say('```');
say('DV_i  = y_i − mateY_i        （mateY_i = 其余 9 人均值）');
say('F_ij  = y_j − mateY_i');
say('Cov(DV_i, F_ij) = Cov(y_i, F_ij) − Cov(mateY_i, F_ij)');
say('  Cov(y_i, F_ij)    = 0 − 0 = 0          （y_i ⊥ y_j；y_i 不在 mateY_i 里）');
say('  Cov(mateY_i, F_ij) = σ²/(n−1) − σ²/(n−1) = 0');
say('  ⟹ 两项均为 0');
say('  ⟹ 独立零假设下 Cov(DV_i, F_ij) ≡ 0,  即该回归的 R² 真值 = 0');
say('```');
say('');
say('> ⚠ **第一版这里写错过**（第二轮审阅发现）：它写 `Cov = 0.111σ²`、`corr = 0.10, R² = 0.01`。');
say('> 那是把 **`Cov(DV_i, DV_j) = −Var(DV)/(n−1) ≈ −0.1235σ²`**（**两名球员的 DV 之间**，且是**负**的）');
say('> 连符号一起搬到了 (DV, 邻居因子) 这一对上。数值验证：`Cov/σ² = −0.0005`（真值 0）。');
say('');
say('> **结论因此更强**：真实 R²（0.52 / 0.64）落在一个**恒为 0** 的量上——');
say('> 分离度是无穷，不是"52 倍"。零分布实测 ≈0.0005 与真值 0 一致。\n');
