...
// 常数改动时这里必须同步；漂移了本脚本就在测别的东西。
const BASE_SHOT_TENDENCY = -4.0, SHOT_FACTOR_GAIN = 2.5;
const SHOT_PRESSURE_NEAR_M = 8.0, SHOT_PRESSURE_SECOND_M = 16.0;
const SHOT_SPACE_MIN_M = 1.5, SHOT_SPACE_MAX_M = 5.0;
...
const scoreOf = (r) => {
  const dq = distanceQuality(r.dGoal);
...
const report = (label, sub) => {
  ...
  // 引擎打分（复合量）
  const sScore = S.map(scoreOf); const pScore = P.map(scoreOf);
  const sa = [...sScore].sort((x, y) => x - y); const sb = [...pScore].sort((x, y) => x - y);
  const A = auc(sScore, pScore);
  console.log(`   ${'★引擎射门打分'.padEnd(21)} ${qs(sa, 0.5).toFixed(2).padStart(9)} ${qs(sb, 0.5).toFixed(2).padStart(9)} ${A.toFixed(3).padStart(7)}`);
  out.engineScore = A;
