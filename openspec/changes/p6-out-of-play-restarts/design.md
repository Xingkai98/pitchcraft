...
| 射门被扑出 → 底线 | result=saved + 扑出（未扑住）+ **未钳制弹开点越门线** | 角球（进攻方） |
...
- **扑出底线角球**：`emit_shot_highlight` 的 saved-rebound 分支算**未钳制**弹开点；若越门线（home 攻 x>1 / away 攻 x<0）→ 新增 `HighlightOutcome::CornerAward`（角球）；否则维持现有松散球（rebound）。**角球触发源 = 扑出反弹越线**；按此源角球约 1-2 次/场（19 射门×30% saved×30% 扑出×~30% 越线 ≈ 0.5-2 次），不设 6-10 次/场的高频目标。
...
**为什么**：界外球 = 从边线短传重新组织，复用 pass 高亮 + 掷球者定位。
...
- viewer 据 h 调球大小；未提供 h 时默认按距离插值（向后兼容）。

**为什么**：P6 首批已实现"球大小表示高度"，但协议无 h 字段、viewer 硬编码——补协议字段让角球/界外球/头球能区分高度。
