...
| 测试 | 位置 | 性质 | 处理 |
|---|---|---|---|
| `v2_tackle_frequency_in_target_range` | P7 槽位时代遗留带 `[3,14]`（槽位层 P31 已删） | 换成同源体量带 `[24,50]` + 写明「语义已死」 |
| `p30_window_foul_cancels_without_shot` | 打分基线变更使原几何选出 Tackle | 重扫几何取 d≈3.15m（Foul 胜出） |
| `l2_sent_off_kickoff_seeds` | RNG 重排使原 4 seed 零红牌 | 重扫取「红牌先于进球」的 29/35/59/84 |
