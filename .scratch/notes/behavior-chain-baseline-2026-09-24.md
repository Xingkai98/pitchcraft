...
- 源码 commit：`baef134a3e422492c763aa95f502d888f14cadc4`（`engine/src/` 未改动）
- `MODEL_VERSION` = 6，engine crate 0.1.0
- sidecar schema 指纹：`fnv1a64:7c76518ceff85604`
- **引擎源码指纹**：`fnv1a64:883f16287059d32a`（哈希编译进二进制的 `lib.rs`+`observation.rs`+`rng.rs`
...
样本量（300 场 × 90 分钟，与 #15A 校准门逐数字一致）：331,966 `ControlFact`、26,429 episode、
14,476 restart、15,743 争抢、1,748,484 事件（其中 `beat` 1,608,970、决策动作 138,602）、
...
| 头条数字 | 值 | 含义 |
|---|---:|---|
| possession 内相邻动作间隔 | **12.52 s**（P50 10.9，P90 16.6，最大 42） | 出球由行动机会 deadline 驱动 |
| 争抢时长 = 0 s 的占比 | **66.0%**（10,395 / 15,743） | 松散球不是竞争状态 |
| 争抢时长的全部取值 | **{0,1,2,…,9}** 十档 | 与球员速度无关的离散档位 |
| 拦截/抢断后原队夺回率 | **0.0%**（7979 / 1417 次，无一例外） | 归属由丢球方式预定 |
| 传球 `lost` 后原队夺回率 | **93.5%**（2132 次） | 「失败传球」不造成失球 |
| 任意球准备期 | 恒 **1.0 s**（7126 次，场间 sd 0） | 球位恰在发球者脚下 |
| 门球准备期 | 恒 **0.0 s**（3405 次） | 结构上跳过准备期 |
| 争抢位置落在**中央 40% 区间** `x∈[0.3,0.7]` | **85.0%**（五等分带 `[451, 5264, 4120, 5415, 493]`） | 两端各 20% 的区间合计仅 6%，丢球几乎不发生在两端 |
...
累计 possession 时长 5141.8 s / 5400 s = **95.2%** 的墙钟时间处在某队明确控制之下，
其中每段平均 58.9 s 却只包含 4.76 个决策动作。
...
### A1 持球-出球节奏：相邻动作间隔 12.52 s（high）

- **证据**：99,775 个 episode 内动作间隔；逐场均值再跨场平均 12.52 s，P50 10.90、P90 16.56、最大 42。
- **回放**：seed 76 @ 2842 s episode 48（事件 3092/3103/3148/3159/3177/3187，最大间隔 42 s）；
  seed 204 @ 4851 s episode 77；seed 107 @ 1191 s episode 20。
- **为什么不像足球**：持球时长与场面脱钩，且动作数被压到 4.76——真实足球里持球时长由压迫、
  接应选项和推进机会决定。
- **机制（已读代码核对 + 变异实证，2026-09-24 更正）**：**不是** `HOLD_MIN_TICKS`/`HOLD_MAX_TICKS`
  ——它们在 P31 删槽位后**已无任何读取点**（`lib.rs` 只剩声明；改它们不改变任何输出）。
  真实驱动是 P28/P31 的**行动机会 deadline**：`advance_action_opportunity`（lib.rs:2225）
  只在 `opp.age_ticks >= opp.deadline_ticks` 时才让 `build_action_plan` 产出一次决策，
  deadline 由 `action_deadline_for`（lib.rs:1731）→ `compute_action_deadline`（lib.rs:1619）
  从 `BASE_ACTION_DEADLINE_TICKS=7`（lib.rs:1077）按危险/压迫/出球空间压缩到
  `MIN/MAX_ACTION_DEADLINE_TICKS=3/12`（lib.rs:1085-1086，门将 +3，lib.rs:1088）；
  结算为「继续带球」时会**重新开一个机会再等一轮**，故间隔是若干轮 deadline 之和——
  这解释了 P90 达 16.6 s、最大 42 s，而不是停在 12 s 上界。
  **变异实证**（本轮真跑，`lib.rs` 已还原为 pristine）：把 `BASE_ACTION_DEADLINE_TICKS`
  从 7 改成 3 后，同一 30 seed 上间隔 **12.52 s → 8.50 s**（P50 10.90 → 7.15，P90 16.56 → 14.00），
  episode 动作数 **4.76 → 5.54**、episode 数 2604 → 3343。这同时证明了两件事：
  ① 该常量确实是本指标的驱动（不是死常量）；
  ② **即使把它压到 3，间隔仍停在 8.50 s**——因为 `MIN_ACTION_DEADLINE_TICKS=3` 是硬下界，
...
### A2 争抢时长退化：66% 的松散球同 tick 被拾回（high）

- **证据**：15,743 次争抢，时长取值只有十档：`0s=10395, 1s=146, 2s=2, 3s=5015, 4s=96, 5s=31,
  6s=21, 7s=23, 8s=11, 9s=3`。逐场口径下各原因时长：`interception_loose=0.00s`（sd 0）、
  `tackle_loose=3.00s`（sd 0）、`delivery_loose=1.45s`（P50 1.17，sd 0.52）、`pass_lost=2.65s`。
...
- **机制（两轮审阅后更正）**：时长档位由**「指定的追逐者是否已经在球上」**决定，**不是**由
  ...
  `PICKUP_RADIUS_METERS`（0.5 m）内**（lib.rs:5260）：是则当拍拾取（0 s），否则追逐者要跑过去
  （≈3 tick）。`interception_loose` 恒 0 s 是因为拦截路径**显式把拦截者对账到拦截点**
  （lib.rs:4637「拦截者位置已对账到 at」）；`tackle_loose` 恒 3 s 是因为抢断的松散球点取
  `deflect_point(...)`，把球捅到离抢断者约 5.25 m（`TACKLE_DEFLECT_DISTANCE=0.05` 归一化
  ≈ 5% 球场长）处，追逐者必须先跑过去。`winning_team` 只决定**谁被允许追**，不决定 0 还是 3。
  角球 battle（`start_battle_loose`，lib.rs:5235）仍是唯一双队对等的争抢。
  **本轮已识别但未做的细分**：按 `RestartKind` 拆开 `delivery_loose` 即可判定其 26.3% 来自
  battle 还是门球（门球是该原因最大来源）——留给 #16 之后（不改变 A2 结论：时长仍退化）。
- **缺什么**：#16 空间特征——需要落点附近双方距离才能断言「对手本可赶到」。

### A3 丢球归属由「丢球方式」预定：拦截/抢断 100% 归对手（high）

- **证据**：按争抢原因分解的夺回率极差 **93.5%**（阈值 60%）：
  `interception_loose = 0.0%`（7979）、`tackle_loose = 0.0%`（1417）、
  `pass_lost = 93.5%`（2132）、`delivery_loose = 26.3%`（4172）、`shot_rebound = 5.7%`（35）。
  总夺回率 19.7%（3093 / 15735）——**总率看起来「还算有争抢」，分解后才暴露确定性**。
- **回放**：seed 1 @ 55 s（拦截→对手，事件 58）、seed 1 @ 117 s（传失→原队，事件 125）、
  seed 1 @ 445 s（拦截→对手，事件 478）。
- **为什么不像足球**：同一类事件（拿到松散球）在三种丢球方式下分别是 0%、0%、93.5%，
  说明归属与球落在哪、谁离得近无关，而是由标签预定。直接后果：**拦截与抢断在行为上完全等价**
  （都等于换球权），抢断这个动作不产生任何附加过程。
- **机制（已读代码核对）**：拦截路径 lib.rs:4652 传 `Some(拦截者球队)`、抢断路径 lib.rs:4929
  传 `Some(防守方)` → `nearest_in_team` → 对手不参与；`pass_lost`（lib.rs:4669）与补射
  （lib.rs:4757）传 `None` → `nearest_any`，才有双向比较。
- **缺什么**：#16 空间特征（判断"对手是否本可赶到"）；但 0%/100% 的极端已足以说明
  结果不由位置竞争产生。

### A7 重开准备期：门球 0 s、任意球恒 1 s（high）

- **证据**：逐场口径准备期均值 `corner=15.35s(594, sd 0.74)`、`free_kick=1.00s(7126, sd 0)`、
  `goal_kick=0.00s(3405, sd 0)`、`kickoff=2.58s(612, sd 2.12)`、`throw_in=4.21s(2731, sd 0.16)`；
  **场间 sd 为 0** 的方式有 2 条（`free_kick`、`goal_kick`）。
  交叉证据：`restart_preparation_started = 11071 < restart_taken = 14468`，差 **3397** ≈ 门球数。
...
- **机制（已读代码核对，2026-09-24 更正为两种不同成因）**：
  ① **门球是结构性的**——`goal_kick_started`（observation.rs:2066）在死球**当拍**同时提交
  `dead_ball_started` 与 `restart_taken`，完全不进 `RestartPreparation` 状态，故准备期恒 0。
  ② **任意球恒 1 s 不是常数，而是几何涌现**——准备期本身是「走向球位」的距离循环
  （lib.rs:5047-5065，8 m/step），`min_ticks = if kind == Corner { CORNER_SETUP_MIN_TICKS } else { 0 }`
  （lib.rs:5067）；而 `emit_foul_and_free_kick` 把球位设在**被犯规持球者自己的位置**（`spot = st.pos[carrier]`，
  lib.rs:2327——注意不是"犯规者"，犯规者是事件的 `subject`）、发球者取 `nearest_in_team`，
  于是发球者恒在 1 step + 拾取半径内，下一 tick 即发出。
  真正按 kind 取常数的只有角球（`CORNER_SETUP_MIN_TICKS`，lib.rs:661）。
  **（原报告把两种成因合并成"按 kind 硬编码"，是错的。）**
- **缺什么**：#16 空间特征——需要发球者与球的距离，才能把②的「恒 1 s」从几何解释升级为
  可观测结论；①已由 sidecar 事实计数实证。

### A8 丢球位置集中在中带：85.0%（high，机制 medium）

- **证据**：15,743 次争抢中坐标合法的全部样本，落在 `x ∈ [0.3, 0.7]` 的占 85.0%；
  x 五等分带计数 `[451, 5264, 4120, 5415, 493]`——两端各只占 ~3%。
...
- **机制（未逐条核对，confidence medium）**：传球落点与队形纵向移动范围受限
  （`SIDE_SHIFT_FACTOR` lib.rs:665、`OFF_BALL_RUN_DIST` lib.rs:418）。
...
### A10 失败传球不等于失球：`lost` 之后原队拿回 93.5%（high）

- **证据**：2,132 次 `pass_lost` 争抢中 93.5% 由传球方自己拿回（阈值 **>75% 即异常**，
  即本条判的是"夺回率过高"）。该原因下争抢时长 2.65 s。
...
- **机制（已读代码核对，比原表述更强）**：**结构性偏向传球方**，不是"通常离得近"——
  ...
  提前量点（lib.rs:3735、3908-3911），且**故意不把接球者位置同步到落点**（lib.rs:3905-3907）；
  随后 `start_loose_ball(.., None)`（lib.rs:4669）→ `nearest_any` 取全体最近者，而落点本就是按
  接球者选的，故拾回概率结构性偏高。对比 `intercepted`（lib.rs:4652 传 `Some(拦截者球队)`）：
...
| id | 规则 | 实测 | 阈值 | 结论 |
|---|---|---:|---|---|
| A4 | 重开后立即丢失 | 10.5%（1522 / 14463） | > 25% | 在带内；重开交付多数能形成控制 |
| A5 | 射门前成功传球数 | 4.23（P50 3.01，P90 8.85，n=2760） | < 1.0 | **未触发**——射门前有多次传球，链长不短 |
| A6 | 空转 possession（≥20 s 且动作 ≤1） | 3.29%（870 / 26429） | > 20% | 在带内；动作数 4.76 与 A1 的 12.52 s 间隔自洽 |
| A9 | **开放比赛**成功传球（排除交付）≥2 的 episode 占比 | 57.9% | < 20% | **未触发**——多脚传递是常态 |

A5 与 A9 未触发是本轮最值得记录的一点：它说明"场均链长看起来短"与"链长真的短"是两件事，
只有把口径写成机器规则才能避免把 recon 阶段的印象写进结论。A1 的缺陷体现在**时间**上
（12.52 s 的间隔），而不体现在**动作数**上。

> 口径注 1（A5 分母）：A5 的分母是「动作链里含 `shot` 事件的 episode」（2760），**不是**
...
> 不是开放比赛的推进）。含交付的旧口径为 65.7%，有 2063 个 episode 仅靠"交付 + 一次开放传球"
...
| 指标 | 本轮基线 | 方向 |
|---|---:|---|
| possession 内相邻动作间隔（s） | 12.52 | 降低至 1–3 s 量级 |
| episode 动作数 | 4.76 | 在时长下降的前提下**不降** |
| 争抢时长取值档数 / 0 s 占比 | 10 档 / 66.0% | 档数上升、0 s 占比下降 |
| 拦截/抢断夺回率（各自） | 0.0% / 0.0% | 脱离 0/1 极端 |
| `pass_lost` 夺回率 | 93.5% | 降到与其他丢球方式同一量级 |
| 场间 sd 为 0 的重开准备期方式数 | 2 | 归零（每种方式都有分布） |
| 争抢位置中带占比 | 85.0% | 下移，两端球门区出现丢球 |
| motif `pickup→pass→lost` | 6.63%（779 / 11741） | 与新的球权转移口径一起复核 |
...
① 它直接改善 **A3/A10**（归属预定），两处证据都指向这一个函数；
  **但它不会改善 A2 的主要部分**——A2 里 76.8% 的 0 s 争抢是 `interception_loose`
  （7979/10395），而拦截者本就被引擎对账到球上（距离 0），让对手也来追也拦不住他当拍拾取；
  A2 要另想办法（若要缩短"追过去"的 3 s，得改 `deflect_point` 的弹开距离或拾取半径）；
② 它是局部改动，不触碰射门带与犯规带；
③ 若改动后 A2/A3/A10 仍红，说明前提假设错，可立即回退而不影响其他流程。

**关于 A1 的额外提示（来自本轮变异实证，已二次复测）**：如果 #19 的目标包含把持球节奏拉回
1–3 s，**单改 `BASE_ACTION_DEADLINE_TICKS` 到不了**——实测压到 3（下界由
`MIN_ACTION_DEADLINE_TICKS=3` 兜住）后间隔仍停在 **8.50 s**（P50 7.15、P90 14.00、最大 25；
episode 数 2604 → 3343）。据此推算 12.52 s 的构成：① 约 73%（≈9.1 s）是 deadline 轮次；
② 剩余 ≈3.4 s 是交付飞行 / transition 的 tick。把常数压到 3 后 ① 缩到 ≈4.7 s，
剩下的 **≈3.8 s 仍是 ②**，所以不分别处理「继续带球时重新开机会再等一整轮」
（lib.rs:2225-2262 的重置逻辑）**与** 交付飞行/transition 的固定开销两项，到不了 1–3 s。
（实测的是 12.52 → 8.50 s 这个端到端差值；上面 ①/② 的拆分是按轮次推算的估计，**未单独实测**。）
