...
- **机制（已读代码核对 + 变异实证，2026-09-24 更正）**：**不是** `HOLD_MIN_TICKS`/`HOLD_MAX_TICKS`
  ...
  **变异实证**（**2026-09-25 在 main 基线上重跑**，`lib.rs` 已还原为 pristine）：
  把 `BASE_ACTION_DEADLINE_TICKS` 从 7 改成 3 后，同一 30 seed 上（逐场再跨场平均口径）
  间隔 **11.96 s → 7.98 s**，episode 动作数 **4.73 → 5.13**、episode 数 3075 → 4065。
  这同时证明了两件事：
  ① 该常量确实是本指标的驱动（不是死常量）；
...
**关于 A1 的额外提示（来自变异实证，2026-09-25 在 main 基线上复测）**：如果 #19 的目标包含把
持球节奏拉回 1–3 s，**单改 `BASE_ACTION_DEADLINE_TICKS` 到不了**——实测压到 3（下界由
`MIN_ACTION_DEADLINE_TICKS=3` 兜住）后间隔仍停在约 **7.98 s**（30 seed，逐场再跨场平均口径；
episode 数 3075 → 4065）。据此推算 12.64 s 的构成：① 约 73% 是 deadline 轮次；
② 剩余约 3.4 s 是交付飞行 / transition 的 tick。把常数压到 3 后 ① 显著缩水，
剩下的约 3.8 s 仍是 ②，所以不分别处理「继续带球时重新开机会再等一整轮」
（`advance_action_opportunity` 的重置逻辑）**与** 交付飞行/transition 的固定开销两项，
到不了 1–3 s。（实测的是端到端差值；上面 ①/② 的拆分是按轮次推算的估计，**未单独实测**。
v6 上的对应实测为 12.52 → 8.50 s，结论一致。）
...
- A1 的变异实证在 v7 上复测：`BASE_ACTION_DEADLINE_TICKS` 7→3 使间隔 11.96 → 7.98 s
  （30 seed，逐场再跨场平均），结论与 v6 一致
