...
|------|------|------|------|------|------|
| 主队纵深 | 18.31 [11.55–23.95] | 25.82 [17.05–31.12] | 25.94 [16.76–30.19] | 40.40 [39.53–41.90] | 引擎高于两侧，+56%（vs Metrica）～+121%（vs 主口径） |
| 紧凑度（到重心） | 12.76 [9.70–15.95] | 14.78 [11.55–18.15] | 15.24 [10.75–17.15] | 18.65 [18.31–19.01] | 不重叠，+22%（vs Metrica） |
| 两队重心间距 | 5.24 [3.22–8.32] | 6.88 [5.15–10.57] | 8.32 [6.80–10.54] | 23.53 [23.26–23.86] | 不重叠，**+183%**（vs Metrica） |
| 宽度（主队） | 35.96 [25.17–46.38] | — | 39.58 [28.17–46.78] | 40.85 [40.50–41.18] | **落在真实范围内**，仅报告 |
| 重心到球 | 14.97 [9.81–24.47] | — | 18.36 [16.62–20.38] | 19.87 [12.74–30.59] | 区间重叠，仅报告 |

⚠️ **2026-09-19 修正**（P38 #87）：上表的引擎列此前是**用未合入分支（`demo/off-ball-movement`）
构建的 wasm** 生成的——重心间距偏 −55.8%、宽度偏 −25.9%（旧值 15.11 / 32.45）。
现已用 main 源码重建重算（`viewer/data/benchmark-baseline.json`），并**加了 wasm 来源哨兵**
（`tools/benchmark-baseline.test.mjs`：比对 `engine/src/lib.rs` 的哈希，改引擎不重生成基线即红）。
**真实侧数字不受影响**（那部分只用真实数据）。

**一个关键发现**：Metrica 的纵深（25.94m）**更接近 SkillCorner 的全点值（25.82m）而非其真检测值（18.31m）**——
说明「外推点 / 每帧有效人数」带来的差（~7.5m）**大于数据集之间的差**。**口径选择比数据源更重要。**
（P38 #88 据此把校准靶子定为 **25–26m**，而非主口径的 18.31m——见 wayfinder map #86。）
...
- **真实性统计套件**（`engine/tests/realism.rs`）：
  - L1 规格一致性（`#[ignore]`，verify.sh **第 7 步** release 显式跑，200 seed 聚合）：
    射门三桶比例带（禁区内 15/30/55、弧 7/22/71、远 4/11/85）与头球 chi-square GOF、
    抢断两态成功率 ≈0.5、**射门体量带 `[14,29]`/场**（#104 重标；原 `[6,11]` 是按压缩态标定的）、
    **抢断体量带 `[24,50]`/场**（#104 新增——原测试只有 shot/tackle 比值带，对「两类一起塌缩」是盲的）、
    shot/tackle 比值 `[0.5,1.5]`、角球/界外球均值带与**单场角球上界 `≤15`**（#104 重标，原 `≤12`）、
    犯规/牌带（[16,30] / [2,5] / ≤0.8）、传球成功率 [82%,90%]
    （⚠️ 原描述里的「tackle 稀释模型」「槽位相对 mix」是 P30/P31 删槽位前的遗留，**已不存在**）
